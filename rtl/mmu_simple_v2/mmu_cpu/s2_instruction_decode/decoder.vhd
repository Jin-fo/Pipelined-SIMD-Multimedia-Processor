...
begin
	...
	var_opcode(5 downto 4) := instruc(INSTRUCTION_LENGTH-1 downto INSTRUCTION_LENGTH-2);  
    var_rs3_ptr := (others => '0');
    var_rs2_ptr := (others => '0');
    var_rs1_ptr := (others => '0');
    var_immed   := (others => '0');
    var_rd_ptr  := (others => '0');
    var_wback   := '0';
    var_read    := (others => '0');
	   ...
	   case var_opcode(5 downto 4) is 
			when "00" | "01" => 
				var_opcode(2 downto 0) 	:= instruc(INSTRUCTION_LENGTH-2 downto INSTRUCTION_LENGTH-4);
				var_opcode(4 downto 3)	:= "00";
				var_immed 				:= instruc(INSTRUCTION_LENGTH-5 downto INSTRUCTION_LENGTH-20);
				var_rs3_ptr				:= instruc(INSTRUCTION_LENGTH-21 downto 0);		
				var_read				:= "100";	
				
				var_rd_ptr 				:= instruc(INSTRUCTION_LENGTH-21 downto 0);  
				var_wback 				:= '1';	
				
			when "10" =>
				var_opcode(2 downto 0) := instruc(INSTRUCTION_LENGTH-3 downto INSTRUCTION_LENGTH-5);
				var_opcode(3) := '0'; 
				
				var_rs3_ptr	 			:= instruc(INSTRUCTION_LENGTH-6 downto INSTRUCTION_LENGTH-10);
				var_rs2_ptr				:= instruc(INSTRUCTION_LENGTH-11 downto INSTRUCTION_LENGTH-15);
				var_rs1_ptr	  			:= instruc(INSTRUCTION_LENGTH-16 downto INSTRUCTION_LENGTH-20);
				var_read				:= "111";	
			...
			when "11" => 
				var_opcode(3 downto 0) := instruc(INSTRUCTION_LENGTH-7 downto INSTRUCTION_LENGTH-10);  
				var_rs2_ptr := instruc(INSTRUCTION_LENGTH-11 downto INSTRUCTION_LENGTH-15);
				var_rs1_ptr := instruc(INSTRUCTION_LENGTH-16 downto INSTRUCTION_LENGTH-20);
				var_rd_ptr	:= instruc(INSTRUCTION_LENGTH-21 downto 0);	   
				var_wback 		:= '1';	
				case var_opcode(3 downto 0) is 	
					when "0000" =>
						...
						var_read := (others => '0');
					when "0001" => 	
						var_immed 		:= b"000000000000" & instruc(INSTRUCTION_LENGTH-12 downto INSTRUCTION_LENGTH-15); 
						var_read     := "001";		
					when "1010" =>
						var_immed	:= b"00000000000" & instruc(INSTRUCTION_LENGTH-11 downto INSTRUCTION_LENGTH-15);    
						var_read	:= "001";
					when others => 
						var_read := "011";
				end case;
